...
def go():
    for data in data_list:
        ...
        try:
            ...
            driver.find_element_by_xpath(f'//input[@name="{name}"]').send_keys(data[0])
        ...
        while True:
            try:
                if data[0] in driver.find_element_by_xpath(f'//*[@id="productGrid_table"]/tbody/tr[{i}]/td[3]').text:
                    done.append(driver.find_element_by_xpath(f'//*[@id="productGrid_table"]/tbody/tr[{i}]/td[2]').text)
                    break
                i += 1
            except NoSuchElementException:
