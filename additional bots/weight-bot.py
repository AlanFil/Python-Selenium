...
def go():
    for data in data_list:
        ...
        driver.find_element_by_name('product[search_keywords]').clear()
        driver.find_element_by_name('product[search_keywords]').send_keys(f'{model} {data[0]}')

        driver.find_element_by_css_selector('button[title="Zapisz"]').click()
        done.append(data[0])
